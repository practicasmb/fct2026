    ...
    def _validate_row(
        ...
        for col_idx, header in enumerate(field_names):
            ...
            if field_key in MAX_LENGTHS and len(str_value) > MAX_LENGTHS[field_key]:
                errors.append(
                    ImportRowError(row=row_num, reason=f"Field '{header}' exceeds maximum length of {MAX_LENGTHS[field_key]}")
                )
        ...
        if not POSTAL_CODE_REGEX.match(postal_code):
            errors.append(ImportRowError(row=row_num, reason="Invalid postal code format"))
